    ...
    <div class="toolbar" style="background-color: white">
      <v-row class="px-lg-16 px-0 pt-5" no-gutters>
        <v-col cols="12" class="py-5 px-5 text-lg-h5 text-h6">
          ...
          >
            *Saat ini, data diupdate secara berkala tiap hari pkl 17.00 WIB, sehingga bukan data
            realtime. Pembelian obat selain multivitamin harus menggunakan resep dokter. Sehubungan
            dengan tingginya kebutuhan, ketersediaan obat dapat berubah sewaktu-waktu, harap
        ...
        <v-col cols="12" class="px-5 pb-5 text-center">
          ...
          <span v-else>
            <v-chip v-for="(prov, index) in selected.provinsi" :key="index" outlined class="mx-1">{{
              prov
            }}</v-chip>
          </span>
        </v-col>
        ...
        >
          <v-tooltip top>
            <template v-slot:activator="{ on }">
              <v-btn icon color="gray" v-on="on" @click="sortData()">
                <v-icon v-if="sort == 'ascending'" v-on="on">mdi-sort-ascending</v-icon>
                <v-icon v-else-if="sort == 'descending'" v-on="on">mdi-sort-descending</v-icon>
              </v-btn>
            </template>
            <span>Sort Jumlah: {{ sort }}</span>
          </v-tooltip>
          ...
          <v-row class="pa-5">
            ...
            >
              <div>
                ...
                >
                  <span
                    v-if="apotek.status == 'habis' || formatJumlah(apotek.jumlah) == 0"
                ...
                ><v-card-subtitle
                  ...
                  >{{
                    apotek.obat.toLowerCase() == "multivitamin"
    ...
    methods: {
      ...
      sortData() {
        if (this.sort == "descending") {
          this.sort = "ascending";
          localStorage.setItem("sortItems", this.sort);
          this.refreshPage();
      ...
      formatJumlah(num) {
        ...
        if (num > 999999) {
          ...
          a = (num / 1000).toFixed(1) + " rb";
        } else {
          a = num == null ? 0 : num;
        }

        if (
          a.toString().substring(a.length - 5) == ".0 rb" ||
          a.toString().substring(a.length - 5) == ".0 jt"
        ) {
