        ...
        <v-col cols="12" class="py-5 px-5 text-lg-h5 text-h6 ">
          <v-row>
            <v-col cols="12" md="6" order-md="1" order="2">
              Stok {{ formatTanggal(selected.tanggal) }} <br class="d-block d-md-none" />
              ({{ formatDate(selected.tanggal) }})</v-col
            >
            <v-col
              cols="12"
              md="6"
        ...
        <v-card elevation="5" class="card" height="100%">
          <v-row class="pa-5">
            ...
            <v-col
              lg="5"
              cols="12"
              class="card-right d-flex flex-lg-column flex-row align-lg-end justify-space-between justify-lg-space-between py-0 pt-lg-1 pb-lg-3"
            >
              <div>
                ...
                <v-card-title
                  ...
                  "
                >
                  <span v-if="apotek.jumlah <= 0" class="red--text spaccing"> Habis</span>
                  <span
                    v-else-if="apotek.jumlah <= 20"
                    class="yellow--text text--darken-2 spaccing text-h5 text-md-h4"
                  >
                    Di bawah 20</span
                  >
  ...
  export default {
    ...
    methods: {
      ...
      formatDate(input) {
        ...
        return hari[date - 1] + ", " + tanggal + " " + bulans[parseInt(bulan) - 1] + " " + tahun;
      },
      getLoc(lat, long) {
